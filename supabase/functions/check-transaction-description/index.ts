...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Ты редактор финансовых описаний транзакций. Проверь текст на орфографические и грамматические ошибки на русском языке.

Правила:
- Исправляй только явные ошибки (орфография, грамматика, пунктуация)
- Сохраняй стиль и смысл оригинала
- Не меняй числа, даты, суммы
- Не добавляй новую информацию
- Если ошибок нет, верни оригинальный текст`;
                  ...
                  errors: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        type: { type: "string", enum: ["spelling", "grammar"], description: "Type of error" }
                      },
