-- Fix: create the no-arg is_admin_user() function that expand_tenants_table.sql depends on
-- Then re-apply the failed policy creation from expand_tenants_table.sql

BEGIN;

-- 1) Create the no-arg wrapper (calls has_role with auth.uid())
...
-- 2) Re-apply the policy that failed in expand_tenants_table.sql
-- Add columns first (idempotent - IF NOT EXISTS)
DO $$
BEGIN
...
-- 4) Tenant owner function and policy from the original migration
