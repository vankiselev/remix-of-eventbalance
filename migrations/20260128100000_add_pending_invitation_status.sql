...
-- Добавляем статус приглашения в profiles для контроля доступа
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'profiles_invitation_status_check'
  ) THEN
...
-- 3. Обновляем функцию handle_new_user для установки статуса pending при самостоятельной регистрации
...
-- 4. Функция для приглашения ожидающего пользователя (только для админов)
...
-- 5. Функция для отклонения ожидающего пользователя (удаление аккаунта)
...
-- 6. Функция для получения списка ожидающих пользователей
...
-- 7. Обновляем get_user_profile_with_roles чтобы включать invitation_status
...
BEGIN
  ...
  result := jsonb_build_object(
    'profile', jsonb_build_object(
      ...
      'termination_date', user_profile.termination_date,
      'invitation_status', user_profile.invitation_status
    ),
